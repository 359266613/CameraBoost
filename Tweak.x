...
BOOL enablePauseResume = YES;
BOOL enableTorch = YES;
BOOL enableVideoConfig = YES;
...
static BOOL preferenceBool(CFStringRef key, BOOL defaultValue) {
    CFPropertyListRef value = CFPreferencesCopyAppValue(key, CAMERABOOST_PREF_DOMAIN);
...
static NSInteger preferenceInteger(CFStringRef key, NSInteger defaultValue) {
    CFPropertyListRef value = CFPreferencesCopyAppValue(key, CAMERABOOST_PREF_DOMAIN);
    if (!value) return defaultValue;
...
static void loadCameraBoostPreferences(void) {
    subSecondPrecision = (int)preferenceInteger(CFSTR("CameraBoost_SubSecondPrecision"), 0);
    if (subSecondPrecision < 0 || subSecondPrecision > 3) {
        subSecondPrecision = 0;
    }
    enablePauseResume = preferenceBool(CFSTR("CameraBoost_EnablePauseResume"), YES);
    enableTorch = preferenceBool(CFSTR("CameraBoost_EnableTorch"), YES);
    enableVideoConfig = preferenceBool(CFSTR("CameraBoost_EnableVideoConfig"), YES);
}
...
@interface CAMViewfinderViewController (CameraBoost)
...
@interface CAMBottomBar (CameraBoost)
...
- (void)_startCapturingVideoWithRequest:(id)arg1 {
    %orig;
    if (enableTorch && checkModeAndDevice(self._currentMode, self._currentDevice)) {
...
- (BOOL)_shouldShowIndicatorOfType:(NSUInteger)type forGraphConfiguration:(CAMCaptureGraphConfiguration *)configuration {
    if (enableTorch && type == 0 && checkModeAndDevice(configuration.mode, configuration.device) && ([self._captureController isCapturingVideo] || [self._captureController isCapturingTimelapse])) {
        return YES;
    }
    return %orig;
}
...
- (BOOL)_shouldHideFlashButtonForGraphConfiguration:(CAMCaptureGraphConfiguration *)configuration {
    if (enableTorch && checkModeAndDevice(configuration.mode, configuration.device) && [self._captureController isCapturingVideo]) {
        return NO;
    }
    return %orig;
}
...
- (void)_updateTopBarStyleForGraphConfiguration:(CAMCaptureGraphConfiguration *)configuration capturing:(BOOL)capturing animated:(BOOL)animated {
    %orig(configuration, enableTorch && isBackCamera(configuration.device) ? NO : capturing, animated);
}
...
- (void)_handleUserChangedToFlashMode:(NSInteger)flashMode {
    %orig(enableTorch && flashMode == 2 && isFlashIndicator ? 1 : flashMode);
}
...
- (BOOL)_shouldHideFramerateIndicatorForMode:(NSInteger)mode device:(NSInteger)device {
    return enableVideoConfig ? NO : %orig;
}

- (void)_createFramerateIndicatorViewIfNecessary {
    %orig;
    if (!enableVideoConfig) return;
...
- (void)videoConfigurationStatusIndicatorDidTapFramerate:(id)arg1 {
    if (enableVideoConfig && [self respondsToSelector:@selector(changeVideoConfigurationMode:)]) {
...
- (void)changeVideoConfigurationMode:(UITapGestureRecognizer *)gesture {
    ...
    NSInteger graphDevice = self._currentGraphConfiguration.device;
    NSInteger cameraDevice = graphDevice;
    if (graphDevice > 0 && graphDevice <= (NSInteger)(sizeof(devices) / sizeof(devices[0]))) {
        cameraDevice = devices[graphDevice - 1];
    }
...
- (void)_createVideoControlsIfNecessary {
    %orig;
    if (enablePauseResume && [self respondsToSelector:@selector(_createPauseResumeDuringVideoButtonIfNecessary)]) {
...
- (void)_createPauseResumeDuringVideoButtonIfNecessary {
    ...
    if (@available(iOS 13.0, *)) {
        ...
        NSBundle *bundle = [NSBundle bundleWithPath:ROOT_PATH_NS(@"/Library/Application Support/CameraBoost.bundle")];
